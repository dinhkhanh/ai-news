...
- Social channels are connected per workspace by the members themselves (`/app/channels`, any role that can edit projects): the member who connects a channel is granted it at once and manages it (logo, grants to colleagues, pause, disconnect), as do workspace admins. Platform admins keep a cross-workspace view at `/admin/channels`. OAuth consent by channel owner.
...
3. **Assets.** Visuals in a fixed priority, each tier only for the shots the earlier tiers could not fill: (1) images of the article itself; (2) images from other outlets covering the same story and, at the same rank, footage about the story from video sites (yt-dlp metadata search, section-only download; feature flag); (3) free stock video (Pexels/Pixabay per scene, Haiku ranks thumbnails); (4) AI-generated stills (Gemini image on Vertex; feature flag, spend cap, daily quota, turns the AI disclosure on). Optional web video (flag). Store provider, ID, licence, source URL, hash, dimensions, duration, origin. Dedupe by hash.
...
*Visual priority (2026-09-17):* `prepare-assets` sources pictures article → other outlets (same story) = web videos (same story; media-Lambda `web-video-search` = yt-dlp `ytsearch` + video pages from Firecrawl search, Haiku places them by thumbnail + title, only the picked 5 s segments are downloaded with `web-video` + `trim`, flag `web_video_downloader`) → stock → AI, each tier only for the remaining shortfall (`media/visual-plan.ts`); AI stills come from `gemini-3.1-flash-image` on Vertex (`media/generate.ts`, Imagen predict endpoints were retired in June 2026), gated by flag `ai_media`, the Google Vertex integration + spend cap and the daily `ai_media` quota, at most 8 per build, and they set the project's AI disclosure. The editor's B-roll re-search falls back to AI when stock has nothing.

*Face guard (2026-09-17):* stills from the article, other outlets and editor uploads pass a QA check before they enter a video (flag `face_guard`): Cloud Vision `FACE_DETECTION` once per picture (`media/faces.ts`, result in `assets.meta.frame`), then the pure `media/framing.ts` places the 9:16 cover crop for the scene's own overlays. Three rules: no face cropped by the frame (Ken Burns and punch-in included), no face under the headline, captions, source line, outro, logo or the platforms' bottom block and right rail, and the faces centred horizontally on the upper-third line (the top two thirds are above the fold on Reels / TikTok / Shorts), within ±17 % width / ±12 % height. The guard may pan, zoom up to 1.3× and switch Ken Burns off; the result travels as `focus` on the image visual (composition 0.5.0). A still that cannot pass in its scene leaves the pool and the next candidate or tier fills the shot; it is used anyway, last in the scene and reported in `build_json.framing.forced`, only when nothing else could. The editor re-runs the guard on swap, headline edit and upload and warns on a failing shot. Videos and AI stills (prompted without people) are not scanned.

*Browser capture (2026-09-17):* YouTube refuses downloads from the media Lambda's AWS IP, and a web page cannot read YouTube's media bytes (cross-origin). Picks that fail are stored in `build_json.webVideo.pending`; the editor shows them in a panel and records them in the editor's own Chrome tab (official embed → `getDisplayMedia` current tab → Region Capture crop → `MediaRecorder`), uploads via presigned PUT, `registerWebCapture` transcodes on the Lambda (CFR 30 H.264, exact length) into a `web_video` asset and the clip's 5 s segments replace the tail (stock / AI) shots of the planned scenes; the editor saves a new version as usual. Real time, tab must stay in front, quality = player size × device pixel ratio.

**Phase 5: publishing.** YouTube first; Meta and TikTok as reviews clear. Scheduling, idempotent publish, status polling, disclosure flags, analytics pull + dashboard. *Built 2026-09-16:* channels connected per workspace by a platform admin at `/admin/channels` through signed-state OAuth routes (`/api/channels/oauth/{youtube|meta|tiktok}/{start,callback}`; Google uses the Internal client from env with `youtube.upload` + `youtube.readonly`, Meta and TikTok app credentials come from `/admin/integrations` → Vault; one Meta login yields a channel per Page plus one per linked Instagram account), tokens stored as `channel:<id>` Vault secrets, grants per user, pause / check / disconnect, a 6-hourly cron that refreshes tokens expiring within 24 h, probes the rest and alerts Slack. Publishing (`publications`, one row per attempt with `idempotency_key = pub:<render>:<channel>:<attempt>`): publisher-level role + grant + platform feature flag + daily `publishes` quota, only the finished render of the approved timeline, per-platform metadata defaults from the script's `metadata` block with source attribution and AI-disclosure line (§8), limits validated (title/description/hashtags/duration), privacy, optional schedule (flag; Vietnam wall-clock → `step.sleepUntil`, cancellable via `publication/cancelled`); `publish-publication` uploads once (YouTube resumable upload streamed from R2 with `containsSyntheticMedia`; Facebook Reels start → `file_url` pull → finish; Instagram Reels container → poll → `media_publish`; TikTok Direct Post FILE_UPLOAD in ranged chunks with `is_aigc`, privacy downgraded to the creator's allowed level and recorded), stores the platform post id immediately, polls processing inline for 6 min then hands over to a 10-minute cron; first success moves the project to `published`. Daily 02:30 (VN) analytics pull per channel into `publications.analytics_json` (views/likes/comments/shares + raw); `/app/publications` workspace dashboard, `/admin/analytics` (produced vs published, per platform, per day, top posts, YouTube quota units used today via `usage_costs` provider `youtube_api`). Project page gains an "Đăng" card with per-channel forms, attempt list, cancel / retry. *Changed 2026-09-18:* connecting is self-service. `/app/channels` lets every writer of a workspace link their own accounts through the same OAuth routes (no `?org=`: the workspace is the member's active one, never taken from the query; the signed state carries `from: app|admin` for the way back, and the callback re-checks the membership); the connector is auto-granted, and `canManageChannel` (`src/lib/publish/channel-access.ts`: connector, workspace admin / owner, platform admin) gates logo, grants, pause, token check and disconnect. Reconnecting a live channel refreshes its token without changing who connected it. `/admin/channels` and `/app/channels` share `src/lib/publish/channel-ops.ts`.
