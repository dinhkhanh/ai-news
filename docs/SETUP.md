...
6. Each member connects their own channels at `/app/channels` (**+ YouTube**, signs in with the channel owner's Google account, is granted the channel automatically and can grant colleagues). An admin can do the same for any workspace at `/admin/channels`: pick the workspace → **+ YouTube** (sign in with the channel owner's Google account; the app needs a refresh token, so re-consent is forced) → grant the channel to the publishers who may post. **Check token** proves the Vault round-trip; **Pull analytics** runs the daily pull immediately.
